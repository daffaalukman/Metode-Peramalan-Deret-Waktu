...
```{r}
data <- as.data.frame(read_excel("C:\\Users\\Lenovo\\Downloads\\Data MPDW Week 3.xlsx", sheet = "Sheet1"))
colnames(data) <- c("t","Yt","Xt" )   # Yt = Market share, Xt = Price
str(data)
data
```
...
Dari hasil tersebut terdapat beberapa peubah yang berpengaruh signifikan terhadap taraf nyata 5% yaitu (intercept) , $x_{t-2}$. Adapun keseluruhan model yang terbentuk adalah

$$
\hat{Y_t}=-2.85701+0.54848X_t+...+0.36898X_{t-6}
$$
...
Dari tabel di atas, dapat terlihat bahwa nilai AIC terendah didapat ketika $p=4$ dan $q=1$, yaitu sebesar `-853.6555`. Artinya, model autoregressive optimum didapat ketika $p=4$ dan $q=1$.
...
-   Model 1 : p-value 2e-16 (\< 0.05). Artinya, penambahan lag dari Y (`L(Yt)`) tidak memberikan peningkatan signifikan.\
    Dengan demikian, Model 1 sudah memadai.

-   Model 2 : p-value = 0.09796 (\> 0.01).\
